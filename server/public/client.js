...
// Render/Railway/VPS). Địa chỉ đó được nhúng lúc build vào runtime-config.js
// qua biến môi trường WS_URL. Nếu trống -> dùng same-origin (chạy local,
// hoặc khi chính server này phục vụ luôn giao diện ở /).
function hasConfiguredWsUrl() {
    return !!(window.__OTT_CONFIG__ && String(window.__OTT_CONFIG__.wsUrl || "").trim());
}
...
function getWsBase() {
    ...
    // Chấp nhận cả http(s):// lẫn ws(s):// và bỏ dấu '/' ở cuối cho an toàn.
    return String(window.__OTT_CONFIG__.wsUrl)
        .trim()
        .replace(/\/+$/, "")
        .replace(/^https:/i, "wss:")
        .replace(/^http:/i, "ws:");
...
function connect(room) {
    ...
    ws = new WebSocket(`${getWsBase()}/?room=${encodeURIComponent(currentRoom)}`);
    ...
    // Cảnh báo rõ khi deploy tĩnh mà quên set WS_URL trên Vercel.
    if (!hasConfiguredWsUrl() && !/^(localhost|127\.0\.0\.1)$/.test(location.hostname)) {
        connStatus.textContent = "● Đang kết nối... (chưa cấu hình WS_URL)";
    }
